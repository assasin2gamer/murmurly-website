...
const NodeBackground = () => {
  ...
  return (
    <div className="bg-canvas">
      ...
      <svg   viewBox="0 0 1450 900"
  preserveAspectRatio="xMidYMid meet" className={`engine-svg ${phase === 'prompt' ? 'hidden' : ''}`} >
        {phase !== 'prompt' && (
          <>
            ...
            {NODES.map((node, idx) => {
              ...
              return (
                <motion.g key={node.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.05 }}>
                  <motion.rect 
                    x={node.x} y={node.y} width={NODE_W} 
                    height={HEADER_H + Math.max(node.in?.length || 0, node.out?.length || 0) * ROW_H}
                    fill={COLORS.nodeBody} rx="6" stroke={COLORS.wire} strokeWidth="1"
                    animate={{ opacity: showReport ? 0 : 1 }}
                  />
                  <motion.rect x={node.x} y={node.y} width={NODE_W} height={HEADER_H} fill={COLORS.nodeHeader} rx="6" animate={{ opacity: showReport ? 0 : 1 }} />
                  
                  <foreignObject 
                    x={node.x - 50} 
                    y={node.y - 100} // Move the anchor up to create a "safe zone" for the 3D tilt
                    width={NODE_W + 200} 
                    height={700}
                    style={{ overflow: 'clip', maxHeight:'40vh'}} // Critical for 3D
                    >
                    <div className="fo-inner" style={{ paddingTop: '100px', paddingLeft: '50px' }}>
                        {!showReport ? (
                        <div className="node-content">
                            <div className="node-header-text">{node.title}</div>
                            <div className="node-ports">
                            <div className="p-col">{node.in?.map(l => <div key={l} className="p-row"><div className="dot" />{l}</div>)}</div>
                            <div className="p-col text-right">{node.out?.map(l => <div key={l} className="p-row-rev"><div className="dot" />{l}</div>)}</div>
                            </div>
                        </div>
                        ) : (
                        <motion.div 
                        ...
                        >
                            ...
                            <div className="rep-chart">
                                {[30, 60, 45, 90, 70].map((h, i) => <div key={i} className="bar" style={{height: h + '%'}} />)}
                            </div>
                            </div>
                        </motion.div>
                        )}
